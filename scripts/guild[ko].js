...
db.exec('CREATE TABLE IF NOT EXISTS Guild(name, owner_xuid, owner_name, lv, coda);');
...
addEnum("kick", ["추방"]);
registerOverride("㈜　", [{ type: "enum", name: "추방", optional: false, enum: "kick" }], function () {
	if (this.player) {
		let playerName = this.player.name
		send(this.player, {
			type: "custom_form",
			...
			content: [
				{
					"type": "input",
					"text": "추방할 길드원의 이름을 입력해주세요",
					"placeholder": "온라인 길드원만 추방이 가능합니다"
			...
			ifdeleted1.forEach(name => {
				let ifdeleted2 = db.prepare(`SELECT xuid FROM member WHERE guild = '${name}' AND name = '${target}'`);
				ifdeleted2.forEach(xuid => {
					db.exec(`INSERT INTO kicked VALUES ('${xuid}')`)
					db.exec(`DELETE FROM member WHERE xuid = '${xuid}'`);
					executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ tellraw @s {\"rawtext\":[{\"text\":\"§c길드에서 " + target + "을 추방했습니다!\"}]}");
				});
	...
	if (this.player) {
		...
		ifkicked.forEach(xuid => {
			if (xuid === playerXuid) {
				...
				executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ tellraw @s {\"rawtext\":[{\"text\":\"§c길드에서 추방당했습니다!\"}]}");
...
addEnum("glist", ["g목록o"]);
registerOverride("㈜　", [{ type: "enum", name: "glist", optional: false, enum: "glist" }], function () {
	if (this.player) {
		let playerName = this.player.name;
		let playerXuid = this.player.xuid;
		let memberList = [];
		let findout1 = db.prepare(`SELECT name FROM Guild WHERE owner_xuid = '${playerXuid}'`);
		findout1.forEach(glname => {
			let guildName = glname
			let findout2 = db.prepare(`SELECT name FROM member WHERE guild = '${guildName}'`);
			findout2.forEach(member => {
				memberList.push("\n§r-§b " + member);
			});
		});
		send(this.player, {
			...
			if (data == null) return;
			executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ ㈜　 길드장");
		});
		return null
	}
});
...
registerOverride("㈜　", [{ type: "enum", name: "길드장", optional: false, enum: "owner_guild" }], function () {
	if (this.player) {
		...
		send(this.player, {
			...
			if (data == null) return;
			const playerName = this.player.name;
			if (data == 0) executeCommand("execute @a[name=\"" + playerName + "\",tag=guild_owner] ~ ~ ~ ㈜　 g목록o");
			if (data == 2) executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ ㈜　 추방");
			if (data == 3) send(this.player, {
				type: "custom_form",
				title: "경고",
				content: [
					{
	...
	if (this.player) {
		...
		send(this.player, {
			...
			if (flag) {
				...
				executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ tellraw @a[name=!\"" + playerName + "\"] {\"rawtext\":[{\"text\":\"§a§l" + playerName + "님이 " + guildName + " 길드를 창설하였습니다!\"}]}");
				...
				db.exec(`INSERT INTO Guild VALUES ('${guildName}', '${playerXuid}', '${playerName}', '1', 'open');`);
...
registerOverride("㈜　", [{ type: "enum", name: "가입", optional: false, enum: "search" }], function () {
	if (this.player) {
		let playerName = this.player.name
		let guildList = [];
		let findout1 = db.prepare(`SELECT name FROM Guild`);
		findout1.forEach(guild => {
			guildList.push("\n§r-§a " + guild);
		});
		send(this.player, {
			type: "custom_form",
			...
			searchv.forEach((name, xuid, owner, lv, coda) => {
				send(this.player, {
					type: "custom_form",
					title: "길드 정보",
					content: [
					...
					send(this.player, {
						...
						if (data == 0) executeCommand("execute @a[name=\"" + playerName + "\"] ~ ~ ~ tellraw @a[name=!\"" + playerName + "\"] {\"rawtext\":[{\"text\":\"§a§l" + name + " 길드에 " + playerName +"님이 가입했습니다!\"}]}");
...
console.log("guild[ko].js loaded");
